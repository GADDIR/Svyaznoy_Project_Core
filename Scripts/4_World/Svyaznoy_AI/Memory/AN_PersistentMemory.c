...
class AN_PersistentMemory
{
    ...
    void LoadProfile()
    {
        if (FileExist(m_ProfilePath))
            JsonFileLoader<AN_PersistentMemory>.JsonLoadFile(m_ProfilePath, this)
    }

    // СОХРАНЕНИЕ (Автосейв / Смерть / Выход)
    void SaveProfile()
    {
        JsonFileLoader<AN_PersistentMemory>.JsonMakeFile(m_ProfilePath, this)
    }
